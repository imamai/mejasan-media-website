...
export default async function TermsPage() {
  ...
  const { data } = await sb.from('mejasan_page_content').select('content').eq('page_slug', 'terms').maybeSingle();
  const { lastUpdated, sections } = getPageContent('terms', data?.content) as LegalContent;
