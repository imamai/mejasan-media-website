...
import { ArrowRight, Mic, Trophy, Music, Radio, Monitor, type LucideIcon } from 'lucide-react';
import { createClient } from '@/lib/supabase/server';
import { getPageContent, type EventsContent } from '@/lib/page-content-schema';
...
function iconFor(title: string): LucideIcon {
  ...
  if (t.includes('conference') || t.includes('summit')) return Monitor;
  if (t.includes('launch')) return Mic;
  if (t.includes('award')) return Trophy;
  if (t.includes('concert') || t.includes('performance')) return Music;
  if (t.includes('stream')) return Radio;
  return Monitor;
}
